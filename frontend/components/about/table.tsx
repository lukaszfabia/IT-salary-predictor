...
export default function ExampleData({ amount }: { amount: number }) {
  ...
  const prettyColumns: string[] = columns.map(
    (column: string) =>
      column.charAt(0).toUpperCase() + column.replace(/_/g, " ").slice(1),
  );
          ...
          <TableRow key={rowIndex}>
            {columns.map((column: string, colIndex: number) => (
              <TableCell key={colIndex} className="text-center">
                {Array.isArray(offer[column as keyof Offer])
                  ? (offer[column as keyof Offer] as string[]).join(", ")
                  : offer[column as keyof Offer]}
              </TableCell>
            ))}
          </TableRow>
        ))}
