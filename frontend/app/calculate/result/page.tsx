...
const ResultPage: React.FC = () => {
  ...
  const data = JSON.parse(
    searchParams.get("salaryData") as string
  ) as OutputData;
      ...
      {data ? (
        <div className="max-w-lg mx-auto p-6 bg-transparent rounded-lg shadow-md py-32 text-center">
          ...
          <div className="space-y-2 text-lg">
            <p>
              Predicted salary in city: <b>{data.city}</b> for{" "}
              <b>{data.experience}</b> begin on <b>{data.contractType}</b>,
              working <b>{data.operatingMode}</b>, having knowledge about{" "}
              <b>{data.technologies.join(", ")}</b> is
            </p>
