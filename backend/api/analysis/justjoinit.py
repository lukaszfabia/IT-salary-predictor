...
from backend.api.analysis.db_connect.db import create_connection, get_collection_or_db
...
def save_links(client: MongoClient, driver: WebDriver = webdriver.Chrome()) -> None:
    """insert links to mongoDB

    Args:
        driver (WebDriver, optional): browser that you wanto to use. Defaults to webdriver.Chrome().
    """
    collection = get_collection_or_db(client, "links")
    for value in LINKS.values():
        links = get_links_to_offers(driver, value)
        collection.insert_many([{f"link": link} for link in links])
...
if __name__ == "__main__":
    ...
    client: Collection = create_connection()
    save_links(client)
    raw_offers: Collection = get_collection_or_db(client, "raw_offers")
    links: Collection = get_collection_or_db(client, "links")
    for index, link in enumerate(links.find()):
        try:
            preprocess = PrepareOffer(driver=webdriver.Chrome(), link=link["link"])
            offer = preprocess.get_offer()
            raw_offers.insert_one(offer.__dict__())
        except:
            continue

    # removing preprocessed links
    links.delete_many({})
    print("done")
